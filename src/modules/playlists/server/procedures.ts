...
export const playlistsRouter = createTRPCRouter({
  ...
  addVideo: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!createdPlaylistVideo) throw new TRPCError({code: "BAD_REQUEST"});
    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .select({
          ...
          viewedAt: viewerVideoView.viewAt,
          viewCount: db.$count(videoViews, eq(videoViews.videoId, videos.id)),
          likeCount: db.$count(videoReactions, and(
            eq(videoReactions.videoId, videos.id),
            eq(videoReactions.type, "like")
        ...
        .where(and(
          ...
          cursor 
            ? or( 
                lt(viewerVideoView.viewAt, cursor.viewedAt), 
                and(
                  eq(viewerVideoView.viewAt, cursor.viewedAt),
                  lt(videos.id, cursor.id),
                ),
              ) 
            : undefined,
        )).orderBy(desc(viewerVideoView.viewAt), desc(videos.id))
        // add limit + 1 to check if there is more data
        .limit(limit + 1);
